...
#201-12-12 header printing error of empty line correction, add blood vaf information
...
mis_file=open(this_DB+'_Possible_mising_lineages.txt','w')
cooc_file=open(this_DB+'_cooccur_list.txt','w')
in_line=in_file.readline().strip()
header_line=''
while in_line:
	if in_line[0:4]=='#CHR':
		header_line=in_line
		cooc_file.write(in_line+'\n')
	else:
...
os.system('mkdir -p perLineage2')

for l_id in id_dic.keys():
	out_file=open('./perLineage2/'+l_id+'.subs.txt','w')
